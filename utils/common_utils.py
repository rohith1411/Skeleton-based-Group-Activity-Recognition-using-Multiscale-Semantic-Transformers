...
def collect_results_for_analysis(results, class2idx, 
    ...
    for key in tqdm(results):
        ...
        if person_idx2class:
            ...
            gt_person = results[key]['GT_Person']
            pred_person = results[key]['Pred_Person']
            gt_person_all += results[key]['GT_Person']
            pred_person_all += results[key]['Pred_Person']
            
        person_res.append("{} GT_Person {} Pred_Person {} ".format(key,gt_person, pred_person))
        
        lines.append("{}  GT {} Pred {} Correct? {}".format(key, gt, pred, gt==pred))
        
    #accuracy     
    person_acc_score = accuracy_score(gt_person_all, pred_person_all,normalize=True)
    grp_acc_score =  accuracy_score(gt_all, pred_all,normalize=True)
    
    
    #recall
    person_recall_score = recall_score(gt_person_all, pred_person_all,average='micro')
    grp_recall_score = recall_score(gt_all, pred_all,average='micro')

    #f1 score
    person_f1 = f1_score(gt_person_all, pred_person_all,average='weighted')
    grp_f1 = f1_score(gt_all, pred_all,average='weighted')
   # write results to disk
    ...
    #write_list_of_strings_into_a_txtfile()
    if person_idx2class:
        write_list_of_strings_into_a_txtfile(person_res, result_prefix+'_person.txt')
    ...
    if logger:
            logger.info(f"Accuracy for person action recognition - {person_acc_score}")
            logger.info(f"Recall for person action recognition - {person_recall_score}")
            logger.info(f"f1 score for person action recognition - {person_f1}")
            logger.info(f"Accuracy for group activity recognition - {grp_acc_score}")
            logger.info(f"Recall for group activity recognition - {grp_recall_score}")
            logger.info(f"f1 score for group activity recognition - {grp_f1}")
    return 
