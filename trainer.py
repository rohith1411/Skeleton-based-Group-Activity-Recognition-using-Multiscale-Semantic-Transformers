...
class Trainer():
    ...
    def train(self, epoch):
        ...
        for i, batch_data in enumerate(self.train_loader):
            ...
            # compute assignments
            #N,D = scores_f.shape
            #Y = torch.randn(2,D).cuda()
            with torch.no_grad(): 
                #cluster code assignment
                ...
                q_f = self.wasserstein_distance(scores_f, nmb_iters=self.args.sinkhorn_iterations)
                q_m = self.wasserstein_distance(scores_m, nmb_iters=self.args.sinkhorn_iterations)
                q_c = self.wasserstein_distance(scores_c, nmb_iters=self.args.sinkhorn_iterations)
                q_g = self.wasserstein_distance(scores_g, nmb_iters=self.args.sinkhorn_iterations)
            ...
            loss_thisbatch += lambda_l2 * l2_reg
    ...
    def swap_prediction(self, p_t, p_s, q_t, q_s):
        ...
        loss_kl = F.kl_div(F.log_softmax(p_s, dim=1), q_t, reduction='batchmean')
        loss_kl += F.kl_div(F.log_softmax(p_t, dim=1), q_s, reduction='batchmean')
        loss_kl *= 0.5
        kl_loss_weight = 0.1
        loss = loss_reconstruction + kl_loss_weight * loss_kl
        return loss
    
    def wasserstein_distance(self, scores, epsilon=0.05, nmb_iters=3):
        with torch.no_grad():
                Q = torch.exp(scores / epsilon).t() 
                K, B = Q.shape
                ...
                for _ in range(nmb_iters):
                    M = pairwise_distance(Q, Q, p=2)  # Pairwise distance matrix
                    P = torch.exp(-M / epsilon)  # Pairwise affinity matrix
                    P /= P.sum()  # Normalize the affinity matrix
                    Q *= r.unsqueeze(1)  # Scale Q by the row scaling factor r
                    Q /= torch.sum(Q, dim=0)  # Normalize Q along the columns
                    Q *= (P @ c.unsqueeze(0))  # Multiply Q by the affinity matrix and column scaling factor c
