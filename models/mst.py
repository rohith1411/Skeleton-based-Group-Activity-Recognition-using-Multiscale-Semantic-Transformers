...
from clustering.clust_funcs import kmeans_gpu, dbscan_gpu, affinity_propagation_gpu
import pdb
...
class MST(nn.Module):
    ...
    def forward(self, joint_feats_thisbatch, ball_feats_thisbatch):
        ...
        joint_feats_for_person_thisbatch = joint_feats_thisbatch[:,:,:,:,:self.args.joint2person_feat_dim] #std x and std y coords only 
        joint_img_coords = joint_feats_thisbatch[:,:,:,:,-2:] #raw x,y coords only
        ...
        joint_class_ids = joint_feats_thisbatch[:,:,:,:,-1]  # last dim is the joint class id
        ...
        joint_feats_thisbatch = joint_feats_thisbatch[:,:,:,:,:-1] #storing without raw x,y and joint type
        ...
        # person-person-interaction-track tokens
        ...
        if self.args.dataset_name == 'volleyball':
            ...
            X = person_track_feats_thisbatch_proj.flatten(0,1)
            #X = X.to(device).detach().numpy()
            #while cluster_iterations == 100:
            #cluster_ids_x, cluster_centers = KMeans(n_clusters=2,max_iter=100,n_init=1).fit(X)
            #   cluster_ids_x, cluster_centers, cluster_iterations = kmeans(X=person_track_feats_thisbatch_proj.flatten(0,1), num_clusters=self.args.G, distance='cosine',  tqdm_flag=False, iter_limit=100, device=device)
            k = 2  # Number of clusters
            #cluster_ids_x = kmeans_gpu(X, k)
            eps = 0.3
            min_samples = 3
            #labels = dbscan(X, eps, min_samples)
            labels = affinity_propagation_gpu(X)
            cluster_ids_x = labels.view(person_track_feats_thisbatch_proj.shape[0],-1)
            #cluster_ids_x = cluster_ids_x.view(person_track_feats_thisbatch_proj.shape[0],-1)  # (B, N)
            #person_track_feats_thisbatch_proj.cuda()
        ...
        # object tokens (similar to keypoint tokens)
        ...
        for l in range(self.args.TNT_n_layers):
          ...
          #pred_logit_f.size() - B X No.of.grp-activity classes
        ...
        #scores_n.size() = B X num_prototypes
        ...
        for l in range(self.args.TNT_n_layers):
            ...
            # pred_logit_person.size() - (BXN, No.of.person-action-class)
